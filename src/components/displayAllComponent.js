...
getOptimizeSvg = async () => {
    ...
    for (var setting of this.props.reduxState.svgSettingList) {
        if(this.props.reduxState.svgOptions.length>0){
            this.props.reduxState.displayOptimaze.push({'original':setting.dataurl,'name':setting.name,'fake':await optimizeSvg(setting.dataurl, this.props.reduxState.svgObject.plugins)});
        }else{
            this.props.reduxState.displayOptimaze.push({'original':setting.dataurl,'name':setting.name,'fake':await optimizeSvg(setting.dataurl,null)});
        }
                ...
                <div className='choose'>
                    ...
                    <div className='mpilomshengu'>
                        <NavLink to='/FinalSvgDisplay' >
                            <div className='optimizeButton' onClick={this.getOptimizeSvg}>Optimize</div>
                        </NavLink>
                    </div>
                </div>
            </div>
