...
class FinalSvgDisplay extends Component {
  ...
  downloadOptimizedSvgs = () => {
    ...
    for (let svgDataIndex in svgElementList) {
      ...
      source = source
        .replace('<a class="svg-elem-wrapper" id="id-1">', "")
        .replace("</a>", "")
      ...
      let linkToWrapSvgElements = document.createElement("a");
      document.body.appendChild(linkToWrapSvgElements); // This line makes it work in Firefox.
      linkToWrapSvgElements.setAttribute("href", url);
      linkToWrapSvgElements.setAttribute("download", "test.svg");
      linkToWrapSvgElements.click();
